...
document.addEventListener("load", function () {
    ...
    chrome.storage.local.get(
      ...
      function (items) {
        Object.keys(items).forEach(function (key: string, index: number) {
          ...
          if (typeof items[key as keyof SettingsObj] == "object") {
            const keyInput: HTMLInputElement | null = document.querySelector(`#${key}`);
            if (keyInput) {
              keyInput.value = dictToString(items[key]);
            }
          }
          const toggleAllInput: HTMLInputElement | null = document.querySelector("#toggle-all");
  ...
  resetFormElement.addEventListener("click", saveSettings);
  saveFormElement.addEventListener("click", restoreSettings);
  restoreSettings();
});
