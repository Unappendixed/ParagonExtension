...
function printReport() {
    ...
    // saved.
    var hidden_frame;
    hidden_frame = document.querySelector("#print-frame");
    if (hidden_frame) {
...
function dom_callback(mutation_list, observer) {
    ...
    const canFindAreaField = (_a = doc.querySelector("#f_4")) !== null && _a !== void 0 ? _a : false;
    ...
    const canFindBrokerageField = (_b = doc.querySelector("#hdnf_28")) !== null && _b !== void 0 ? _b : false;
    const shouldShowExpiryNextToCancellation = hotkey_dict["cancellation_shortcut"];
    const canFindCancellationField = (_c = doc.querySelector("#f_209")) !== null && _c !== void 0 ? _c : false;
    ...
    function createRemoveBreakButtons() {
        ...
        if (document.querySelector(doc).find(".whitespace_button").length == 0) {
            ...
            for (let elem of texts) {
                let iter_button = button.cloneNode();
                iter_button.setAttribute('for', elem.id);
                iter_button.addEventListener('click', function (args) {
                    let text_elem = document.querySelector(doc).find(`#${args.target.getAttribute('for')}`);
...
function key_callback(e) {
    ...
    function closeTab() {
        e.preventDefault();
        let lst = window.top.document.querySelectorAll('em[title="Close Tab"]:visible');
...
document.addEventListener('load', function () {
    ...
    var button_style = document.querySelector(`<style>.whitespace_button {float:left;clear:left;display:inline-block;margin-left:120px;}</style>`);
    var privacy_style = document.querySelector(`<style>.privacy {font-weight:bold;text-decoration:underline}.privacy-color {background:goldenrod}</style>`);
